function [ ] = Outlier_detection_linear_nonlinear_regression( X5,Y5 ,dataset_Num)
...
A_linear=IN_data;
B_linear=OUT_data;
coefficient_optimal=Linear_Regression(IN_data,OUT_data);
n=size(IN_data,1);
...
IN_data=X5;
OUT_data=Y5;
coefficient=Non_Linear_Regression(IN_data,OUT_data);
n=size(IN_data,1);
...
for k=1:n 
  ...
  y1=OUT_data;
      B(k,:)=[];
      y1(k,:)=[];
coefficient_temp=Non_Linear_Regression(B,y1);
estimated_y_temp=zeros(n-1,1);
for r=1:n-1
for t=1:size(coefficient_temp,1)   
...
coefficient_non2=Non_Linear_Regression(IN_data,OUT_data);
n=size(IN_data,1);
...
% non linear regression
coefficient_non3=Non_Linear_Regression(IN_data,OUT_data);
n=size(IN_data,1);
% MSE Total
estimated_y=zeros(n,1);
for r=1:n    
...
%  Linear Regression
coefficient_linear_opt=Linear_Regression(IN_data,OUT_data);
n=size(IN_data,1);
